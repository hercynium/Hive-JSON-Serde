...
public class JsonSerDe implements SerDe {
    ...
    public JsonSerDe() throws SerDeException { }
    ...
    public void initialize(Configuration conf, Properties tbl) throws SerDeException {
        ...
        LOG.debug("Initializing JSON SerDe");
        ...
        // if types are missing, default to strings
        ...
        if (columnNames.size() != columnTypes.size()) {
            throw new SerDeException(
                getClass().getName() + ": columns has " + columnNames.size()
                + " elements while columns.types has "  + columnTypes.size() + " elements!"
            );
        }

        LOG.debug(
            getClass().getName() + " initialized with: columnNames="
        ...
        // other configuration
        ignoreMalformedJson = Boolean.parseBoolean(
            tbl.getProperty(PROP_IGNORE_MALFORMED_JSON, "false")
        );
    ...
    private String getSerializedFieldName( List<String> columnNames, int pos, StructField sf) {
        String n = (
          //(columnNames == null || columnNames.size() <= pos) ? sf.getFieldName() : columnNames.get(pos)
          (columnNames.size() <= pos) ? sf.getFieldName() : columnNames.get(pos)
        );
